...
## 🔐 ข้อมูลเก็บที่ไหน + Backup

**ข้อมูลทั้งหมดอยู่บนเครื่องของคุณเอง** — ไม่ถูกส่งออกไปที่ไหน

- ผล​การสแกน, ช่องโหว่, ผู้ใช้, รายงาน → เก็บใน **PostgreSQL บนเครื่องคุณ** (Docker volume `qpent-db-data` ที่ `/var/lib/docker/volumes/...`)
- มีแต่ **การเรียก LLM API** เท่านั้นที่ออกจากเครื่อง — ไปยังผู้ให้บริการที่ **คุณตั้งค่าเอง** (หรือ Ollama ในเครื่อง = ไม่ออกเลย)
- ข้อมูลการสแกน (ซึ่งอ่อนไหว) **ไม่เคยไหลไปหาผู้พัฒนา** 🔒

**ข้อมูลอยู่ทนแค่ไหน:**

| การกระทำ | ข้อมูล |
|---|---|
| `docker compose restart` / รีบูตเครื่อง | ✅ อยู่ครบ |
| `docker compose down` แล้ว `up -d` | ✅ อยู่ครบ |
| `docker compose pull` + `up -d` (อัปเดต) | ✅ อยู่ครบ |
| `docker compose down -v` (มี `-v`) | ❌ ลบข้อมูลทั้งหมด |
...
```
> การสำรองข้อมูลเป็นความรับผิดชอบของผู้ใช้ (ข้อมูลอยู่บนเครื่องคุณ ผู้พัฒนากู้ให้ไม่ได้)
